      ...
      <tr><td align="center">
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:680px;background:#ffffff;border:1px solid rgba(20,33,61,.10);border-radius:24px;overflow:hidden;box-shadow:0 24px 70px rgba(20,33,61,.12)">
          <tr><td style="padding:30px;background:linear-gradient(145deg,#0b1529,#14213d 62%,#22365f);color:#ffffff">
            <div style="font-size:12px;font-weight:800;letter-spacing:.13em;text-transform:uppercase;color:#ffd98a">Biloo website</div>
          ...
          <tr><td style="padding:20px 30px 30px">
            ...
            <a href="mailto:${encodeURIComponent(request.email)}?subject=${encodeURIComponent(`Re: Biloo demo request for ${request.businessName}`)}" style="display:inline-block;margin-top:22px;padding:14px 20px;border-radius:999px;background:#14213d;color:#ffffff;font-size:14px;font-weight:800;text-decoration:none">Reply to customer</a>
          </td></tr>
        </table>
      </td></tr>
    </table>
...
export async function sendDemoRequestEmail(request: DemoRequestMessage): Promise<DeliveryResult> {
  ...
  const from = process.env.DEMO_EMAIL_FROM?.trim() || "Biloo Website <notifications@hisabtech.com>";
  ...
  try {
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        subject: `New Biloo demo request — ${request.businessName}`,
