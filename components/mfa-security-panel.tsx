  ...
  async function startEnrollment() {
    ...
    try {
      ...
      const { data, error: enrollError } = await supabase.auth.mfa.enroll({
        factorType: "totp",
        friendlyName: verified.length ? `Biloo backup authenticator ${verified.length + 1}` : "Biloo primary authenticator",
      });
    ...
    <section className="security-mfa-panel biloo-mfa-manager" aria-labelledby="mfa-heading">
      ...
      {loading ? (
          ...
          <div className="biloo-mfa-enrollment-grid">
            ...
            <div className="biloo-mfa-setup-column">
              <ol className="biloo-mfa-steps">
                ...
                <li><span>2</span><div><strong>Scan this QR code</strong><small>The account will appear as a Biloo authenticator.</small></div></li>
        ...
        <section className="biloo-mfa-empty-state">
          ...
          <div className="biloo-mfa-empty-copy">
            ...
            <span>Even if someone knows your password or gains access to your email provider, protected Biloo actions still require a code generated on your device.</span>
        ...
        <section className="biloo-mfa-ready-state">
          ...
          <div><p>STRONG ASSURANCE ACTIVE</p><h3>This browser session is protected.</h3><small>Biloo can now complete privileged actions permitted by your organization role.</small></div>
      ...
      {verified.length ? (
        <section className="biloo-mfa-factor-section" aria-labelledby="verified-authenticators-heading">
          ...
          {required && verified.length === 1 ? <p className="biloo-mfa-factor-tip"><Icon name="lightbulb" size={15} /> Add a backup authenticator before replacing your current device. Biloo will not remove the only factor required by your role.</p> : null}
        </section>
      ) : null}

      <footer className="biloo-mfa-footer-note"><Icon name="lock" size={15} /><span>Authenticator secrets are handled through Supabase MFA. Biloo stores verification state and audit events, not reusable one-time codes.</span></footer>
    </section>
  );
}
